...
public:
    ...
    vector<LogicLayer<HBM>*> logic_layers;
    HBM * spec;

    vector<int> addr_bits;
    vector<int> requests_per_chennel;
    int tx_bits;
    int cacheline_size;
          ...
          addr_bits(int(HBM::Level::MAX))
    {
        ...
        for (int i = 0 ; i < stacks ; ++i) {
          logic_layers.emplace_back(new LogicLayer<HBM>(configs, i, spec, ctrls,
              this, std::bind(&Memory<HBM>::receive_packets, this,
                              std::placeholders::_1)));
        }
    ...
    void record_core(int coreid) {
      // TODO record multicore statistics  //satanu todo
    }
    ...
    Packet form_request_packet(const Request& req) {
      ...
      long adrs = addr;
      int max_block_bits = spec->flit_num_bits;
      ...
      int lng = req.type == Request::Type::READ ? 1 : 1 +  spec->payload_flits;
      Packet::Command cmd;
      switch (int(req.type)) {
    ...
    void receive_packets(Packet packet) {
      ...
      req.depart_memory = clk;
      if (req.type == Request::Type::READ) {
        read_latency_sum += req.depart_memory - req.arrive_memory;
        request_packet_latency_sum += req.arrive - req.arrive_memory;
        response_packet_latency_sum += req.depart_memory - req.depart;
        req.callback(req);
      }
    }
    ...
    bool send(Request req)
    {
        ...
        requests_per_chennel[req.addr_vec[int(HBM::Level::Channel)]]++;
        req.arrive_memory = clk;

        if(req.from_nmp){
            Packet packet = form_request_packet(req);
            ctrls[req.addr_vec[int(HBM::Level::Channel)]] -> receive(packet);
    ...
    {
        int pending = 0;
        for (auto link : logic_layers[0]->host_links)
            pending = pending + link->slave.input_buffer.size() + link->master.output_buffer.size();

        for (auto link : logic_layers[0]->pass_thru_links)
            pending = pending + link->slave.input_buffer.size() + link->master.output_buffer.size();

        for (auto ctrl : logic_layers[0]->xbar.ctrls)
            pending = pending + ctrl->response_packets_buffer.size() + ctrl->incoming_packets_buffer.size() + ctrl->pending.size();
    ...
    void finish(void) {
      dram_capacity = max_address;
      int *sz = spec->org_entry.count;
      maximum_internal_bandwidth = spec->speed_entry.rate * 1e6 * spec->channel_width * sz[int(HBM::Level::Channel)] / 8;
      maximum_link_bandwidth = spec->link_width * 2 * spec->source_links * spec->lane_speed * 1e9 / 8;
    ...
    long get_memory_transection_info() {
        return ((read_transaction_bytes.value() * 1e9) + (write_transaction_bytes.value() * 1e9));
    }
    ...
    long page_allocator(long addr, int coreid) {
        ...
        switch(int(translation)) {
            ...
            case int(Translation::Random): {
                ...
                // TODO: page size should not always be fixed to 4KB
...
template<>
class Memory<HBM2, Controller> : public MemoryBase
{
public:
  long max_address;
  long capacity_per_stack;
...
public:
    ...
    vector<Controller<HBM2>*> ctrls;
    vector<LogicLayer<HBM2>*> logic_layers;
    HBM2 * spec;
    ...
    Memory(const Config& configs, vector<Controller<HBM2>*> ctrls)
        : ctrls(ctrls),
          spec(ctrls[0]->channel->spec),
          addr_bits(int(HBM2::Level::MAX))
    {
        // make sure 2^N channels/ranks
        // TODO support channel number that is not powers of 2
        int *sz = spec->org_entry.count;
        ...
        int stacks = configs.get_int_value("stacks");
        for (int i = 0 ; i < stacks ; ++i) {
          logic_layers.emplace_back(new LogicLayer<HBM2>(configs, i, spec, ctrls,
              this, std::bind(&Memory<HBM2>::receive_packets, this,
                              std::placeholders::_1)));
        }
    ...
    bool send(Request req)
    {
        ...
        switch(int(type)){
            ...
            case int(Type::RoBaRaCoCh):
                req.addr_vec[0] = slice_lower_bits(addr, addr_bits[0]);
                req.addr_vec[addr_bits.size() - 1] = slice_lower_bits(addr, addr_bits[addr_bits.size() - 1]);
                for (int i = 1; i <= int(HBM2::Level::Row); i++)
                    req.addr_vec[i] = slice_lower_bits(addr, addr_bits[i]);
                break;
            default:
                assert(false);
        ...
        requests_per_chennel[req.addr_vec[int(HBM2::Level::Channel)]]++;
        req.arrive_memory = clk;

        if(req.from_nmp){
            Packet packet = form_request_packet(req);
            ctrls[req.addr_vec[int(HBM2::Level::Channel)]] -> receive(packet);

            if (req.type == Request::Type::READ) {
                ++num_read_requests[coreid];
                ++incoming_read_reqs_per_channel[req.addr_vec[int(HBM2::Level::Channel)]];
            }
            if (req.type == Request::Type::WRITE) {
                ++num_write_requests[coreid];
            }
            ++incoming_requests_per_channel[req.addr_vec[int(HBM2::Level::Channel)]];
            ++mem_req_count;
            return true;
        }
        else{
    ...
    void finish(void) {
      dram_capacity = max_address;
      int *sz = spec->org_entry.count;
      maximum_internal_bandwidth = spec->speed_entry.rate * 1e6 * spec->channel_width * sz[int(HBM2::Level::Channel)] / 8;
      maximum_link_bandwidth = spec->link_width * 2 * spec->source_links * spec->lane_speed * 1e9 / 8;
      long dram_cycles = num_dram_cycles.value();
      long total_read_req = num_read_requests.total();
      for (auto ctrl : ctrls) {
...
public:
    ...
    void receive_packets(Packet packet) {
      ...
      if (req.type == Request::Type::READ) {
        read_latency_sum += req.depart_memory - req.arrive_memory;
        debug_hmc("read_latency: %ld", req.depart_memory - req.arrive_memory);
        request_packet_latency_sum += req.arrive - req.arrive_memory;
        debug_hmc("request_packet_latency: %ld", req.arrive - req.arrive_memory);
        response_packet_latency_sum += req.depart_memory - req.depart;
        debug_hmc("response_packet_latency: %ld", req.depart_memory - req.depart);
        req.callback(req);
      }
    }
    ...
    int pending_link_packets()
    {
        ...
        for (auto vault_ctrl : logic_layers[0]->xbar.ctrls)
            pending = pending + vault_ctrl->response_packets_buffer.size() + vault_ctrl->incoming_packets_buffer.size() + vault_ctrl->pending.size();
