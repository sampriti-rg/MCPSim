#ifndef __3DMEMORY_CONTROLLER_H
#define __3DMEMORY_CONTROLLER_H
...
public:
        ...
        refresh(new Refresh<HBM>(this))
    {
        ...
        if (record_cmd_trace){
            ...
            cmd_trace_file.open(
                "traces/ctrl/" + cmd_trace_prefix + "chan-" + to_string(channel->id)
                + ".cmdtrace");
        }
        if (configs.get_dram_power_config() != "NULL") {
          with_drampower = true;
          drampower = new libDRAMPower(
              Data::MemSpecParser::getMemSpecFromXML(
                  configs.get_dram_power_config()),
              true);
        }
...
public:
    ...
    long clk = 0;
    DRAM<HBM2>* channel;
    Scheduler<HBM2>* scheduler;  // determines the highest priority request whose commands will be issued
    RowPolicy<HBM2>* rowpolicy;  // determines the row-policy (e.g., closed-row vs. open-row)
    RowTable<HBM2>* rowtable;  // tracks metadata about rows (e.g., which are open and for how long)
    Refresh<HBM2>* refresh;
    ...
    // bool pim_mode_enabled = false;
    /* Constructor */
    Controller(const Config& configs, DRAM<HBM2>* channel) :
        channel(channel),
        scheduler(new Scheduler<HBM2>(this)),
        rowpolicy(new RowPolicy<HBM2>(this)),
        rowtable(new RowTable<HBM2>(this)),
        refresh(new Refresh<HBM2>(this))
    {
        record_cmd_trace = configs.record_cmd_trace();
        print_cmd_trace = configs.print_cmd_trace();
        if (record_cmd_trace){
        ...
        if (configs["no_DRAM_latency"] == "true") {
          no_DRAM_latency = true;
          scheduler->type = Scheduler<HBM2>::Type::FRFCFS;
        }
        if (configs["unlimit_bandwidth"] == "true") {
          unlimit_bandwidth = true;
          printf("nBL: %d\n", channel->spec->speed_entry.nBL);
    ...
    bool is_ready(list<Request>::iterator req)
    {
        typename HBM2::Command cmd = get_first_cmd(req);
        return channel->check(cmd, req->addr_vec.data(), clk);
    }

    bool is_ready(typename HBM2::Command cmd, const vector<int>& addr_vec)
    {
        return channel->check(cmd, addr_vec.data(), clk);
    }
    ...
    {
        // cmd must be decided by the request type, not the first cmd
        typename HBM2::Command cmd = channel->spec->translate[int(req->type)];
        return channel->check_row_hit(cmd, req->addr_vec.data());
    }

    bool is_row_hit(typename HBM2::Command cmd, const vector<int>& addr_vec)
    {
        return channel->check_row_hit(cmd, addr_vec.data());
    }
    ...
    bool is_row_open(typename HBM2::Command cmd, const vector<int>& addr_vec)
    {
        return channel->check_row_open(cmd, addr_vec.data());
    }
...
private:
    typename HBM2::Command get_first_cmd(list<Request>::iterator req)
    {
        typename HBM2::Command cmd = channel->spec->translate[int(req->type)];
        if (!no_DRAM_latency) {
          return channel->decode(cmd, req->addr_vec.data());
    ...
    void issue_cmd(typename HBM2::Command cmd, const vector<int>& addr_vec)
    {
        // update power estimation
        if (with_drampower) {

          const string& cmd_name = channel->spec->command_name[int(cmd)];
          int bank_id = addr_vec[int(HBM2::Level::Bank)];
          bank_id += addr_vec[int(HBM2::Level::Bank) - 1] * channel->spec->org_entry.count[int(HBM2::Level::Bank)];
          printf("%ld %d\n", clk, bank_id);
          drampower->doCommand(Data::MemCommand::getTypeFromName(cmd_name), bank_id, clk);
        ...
        if (print_cmd_trace){
            printf("%5s %10ld:", channel->spec->command_name[int(cmd)].c_str(), clk);
            for (int lev = 0; lev < int(HBM2::Level::MAX); lev++)
                printf(" %5d", addr_vec[lev]);
            printf("\n");
        }
        assert(is_ready(cmd, addr_vec));
    ...
    vector<int> get_addr_vec(typename HBM2::Command cmd, list<Request>::iterator req){
        return req->addr_vec;
    }
};
